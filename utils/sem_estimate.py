...
def fcns4sem(dyn_graph: object, D_obs: OrderedDict) -> list[dict]:
    ...
    Returns:
        list[dict]: two dict in the list, respectively contains the necessary emission and transition functions
        to construct the SEM model
    """
    the_graph = dyn_graph.graph
    fcns = {t: OrderedDict() for t in range(len(dyn_graph.full_output_vars))}
    ...
    for node in sorted_nodes:
        ...
        if not predecessors:
            ...
            index_ini = tf.expand_dims(obs_data_x[0], 0)
...
def sem_hat(dyn_graph: object, D_obs: OrderedDict) -> classmethod:

    class SEMhat:
        def __init__(self):
            self.fcns = fcns4sem(dyn_graph, D_obs)
            self.sorted_nodes = list(self.fcns[0].keys())

        def static(self):
            f = OrderedDict()
            for node in self.sorted_nodes:
                # f[node] = lambda sample: self.fcns[0][node](sample)
        ...
        def dynamic(self):
            f = OrderedDict()
            for node in self.sorted_nodes:
                f[node] = (lambda node: lambda t, sample: self.fcns[t][node](sample))(node)
            return f
