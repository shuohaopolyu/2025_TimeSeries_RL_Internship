...
class IndepedentGaussians:

    def __init__(self, mus:tf.Tensor, sigmas:tf.Tensor):
        self.mus = mus
        self.sigmas = sigmas
        assert len(mus.shape) == 1 and len(sigmas.shape) == 1, "mus and sigmas must be 1D tensors"
        assert mus.shape[0] == sigmas.shape[0], "mus and sigmas must have the same length"
